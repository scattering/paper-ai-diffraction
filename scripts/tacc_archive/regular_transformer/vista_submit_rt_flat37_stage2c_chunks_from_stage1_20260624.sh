...
epoch1_job="$(submit_job -A "$ACCOUNT" -p "$PARTITION" --dependency="afterok:${STAGE1_JOB_ID}" \
  --export="ALL,TARGET_NUM_EPOCHS=1,RESUME_CHECKPOINT=${STAGE1_BEST},RESUME_WEIGHTS_ONLY=1" \
  vista_train_rt_flat37_stage2c_epoch_chunk_20260624.sh)"

epoch2_job="$(submit_job -A "$ACCOUNT" -p "$PARTITION" --dependency="afterok:${epoch1_job}" \
  --export="ALL,TARGET_NUM_EPOCHS=2,RESUME_CHECKPOINT=${STAGE2_LATEST},RESUME_WEIGHTS_ONLY=0" \
  vista_train_rt_flat37_stage2c_epoch_chunk_20260624.sh)"

epoch3_job="$(submit_job -A "$ACCOUNT" -p "$PARTITION" --dependency="afterok:${epoch2_job}" \
  --export="ALL,TARGET_NUM_EPOCHS=3,RESUME_CHECKPOINT=${STAGE2_LATEST},RESUME_WEIGHTS_ONLY=0" \
  vista_train_rt_flat37_stage2c_epoch_chunk_20260624.sh)"

eval_job="$(submit_job -A "$ACCOUNT" -p "$EVAL_PARTITION" --dependency="afterok:${epoch3_job}" \
  vista_eval_rt_flat37_stage2c_20260624.sh)"

echo "RT stage2 epoch1 job: ${epoch1_job}"
echo "RT stage2 epoch2 job: ${epoch2_job}"
echo "RT stage2 epoch3 job: ${epoch3_job}"
