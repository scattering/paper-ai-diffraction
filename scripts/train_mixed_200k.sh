...
BASE_CONFIG="${BASE_CONFIG:-$ROOT/configs/mixed_200k_pilot.json}"
MIXED_TRAINREADY_H5="${MIXED_TRAINREADY_H5:-}"
...
if [[ -z "$MIXED_TRAINREADY_H5" ]]; then
  echo "Set MIXED_TRAINREADY_H5 before running." >&2
  exit 2
fi

mkdir -p "$MODEL_OUTDIR" "$ROOT/results/tmp"
TMP_CONFIG="$(mktemp "$ROOT/results/tmp/mixed200k_XXXX.json")"
trap 'rm -f "$TMP_CONFIG"' EXIT

ARGS=(
  --base-config "$BASE_CONFIG"
  --output-config "$TMP_CONFIG"
  --set "data_path=$MIXED_TRAINREADY_H5"
